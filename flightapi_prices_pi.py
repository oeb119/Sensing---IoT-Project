...
## Converts price from usd to gbp
def get_in_gbp(usd_value):
    currency_api = "cur_live_kCCzxXJkprT4xafExoVYvfV8Lo1HaOMtNSmChYqn"
    currency_endpoint = "https://api.currencyapi.com/v3/latest?apikey="+str(currency_api)
    response = requests.get(currency_endpoint).json()
    gbp_rate = response["data"]["GBP"]["value"]
    gbp_value = usd_value*gbp_rate
    return gbp_value
...
def min_flight_price(file_name):
    ...
    min_price_gbp = get_in_gbp(min_price)
